...
class ObjectDetectionIngestModuleFactory(IngestModuleFactoryAdapter):

    # TODO: give it a unique name.  Will be shown in module list, logs, etc.
    moduleName = "Object Detector"
    ...
    def getModuleDescription(self):
        return "Object detection using YOLOv3"
    ...
    def createDataSourceIngestModule(self, ingestOptions):
        ...
        return ObjectDetectionIngestModule()


# Data Source-level ingest module.  One gets created per data source.
# TODO: Rename this to something more specific. Could just remove "Factory" from above name.
class ObjectDetectionIngestModule(DataSourceIngestModule):

    _logger = Logger.getLogger(ObjectDetectionIngestModuleFactory.moduleName)
    ...
    def startUp(self, context):
        exe_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "model\\detecta_pessoa_carro_faca.exe")
    ...
    def process(self, dataSource, progressBar):
        ...
        shutil.rmtree(str(Case.getCurrentCase().getCaseDirectory() + "Relatorio"), ignore_errors=True)
        shutil.rmtree(str(Case.getCurrentCase().getCaseDirectory() + "Deteccoes"), ignore_errors=True)
        ...
        arq_lista_images = open(reportsDir + "\saida.csv", "w")
        ...
        # Obtem nomes dos arquivos marcadados, ou seja, que contem objetos de interesse

        #path_marcados_pessoa = os.path.join(Case.getCurrentCase().getCaseDirectory(), "temporario")
        #self.log(Level.WARNING, "-------------------------------------------------DIRETORIO: " + str(path_marcados_pessoa))

        # Acessa arquivo csv contendo o objeto identificado e o path do arquivo de imagem orignal


        arquivo_csv_objetos = open(reportsDir + "\Relatorio_deteccoes.csv","r") 
        spamreader = csv.reader(arquivo_csv_objetos, delimiter=';')
        fileManager = Case.getCurrentCase().getServices().getFileManager()
        
        for row in spamreader:
            #arq_pessoas.append(file)
            #arquivo = fileManager.findFiles(filename, path_marcados_pessoa)
            path, filename = os.path.split(row[1])
            ...
            if arquivo:
                self.log(Level.WARNING, "------------------------------------------------- ACHOU ARQUIVO +++++++++++++++++++++++++++++++++")

                if (row[0] == "person"):
                    art = arquivo[0].newArtifact(BlackboardArtifact.ARTIFACT_TYPE.TSK_INTERESTING_FILE_HIT)
                    att = BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_SET_NAME, ObjectDetectionIngestModuleFactory.moduleName, "Pessoa")
                    ...
                    art = arquivo[0].newArtifact(BlackboardArtifact.ARTIFACT_TYPE.TSK_INTERESTING_FILE_HIT)
                    att = BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_SET_NAME, ObjectDetectionIngestModuleFactory.moduleName, "Carro")
                    art.addAttribute(att)
                    ...
                    art = arquivo[0].newArtifact(BlackboardArtifact.ARTIFACT_TYPE.TSK_INTERESTING_FILE_HIT)
                    att = BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_SET_NAME, ObjectDetectionIngestModuleFactory.moduleName, "Armas")
                    art.addAttribute(att)
        ...
        for line in lines:           
            ...
            if arquivo:
                art = arquivo[0].newArtifact(BlackboardArtifact.ARTIFACT_TYPE.TSK_INTERESTING_FILE_HIT)
                att = BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_SET_NAME, ImageClassificationIngestModuleFactory.moduleName, "Documentos_gerais")
                art.addAttribute(att)
